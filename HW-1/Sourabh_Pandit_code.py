...
class LogisticRegression:
    ...
    def fit(self, X: np.ndarray, y: np.ndarray) -> list[float]:
        ...
        for _ in range(1, self.max_iter):
            ...
            dw = np.dot(X_scaled.T, (y_pred -y)) / n_samples
...
def grid_search_linreg() -> None:
    ...
    for iter in (500, 1000, 5000):
        for lr in reversed((0.1, 0.08, 0.06, 0.04, 0.02, 0.01, 0.008, 0.006, 0.004, 0.002, 0.001, 0.0008, 0.0006, 0.0004, 0.0002, 0.0001)):
            for l2_lambda in reversed((0.1, 0.08, 0.05, 0.02, 0.01, 0.008, 0.005, 0.002, 0.001, 0.0008, 0.0004, 0.0002, 0.0001, 0.00008, 0.00006, 0.00004, 0.00002)):
                ...
                if rmse < 76.0:
                    print(f"[LR: {lr:6.4f}, max_iter: {iter:7d} lambda: {l2_lambda:6.4f}]", end=' --> ')
                print("RMSE:", rmse)
