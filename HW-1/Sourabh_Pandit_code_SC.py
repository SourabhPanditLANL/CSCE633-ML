...
class LinearRegression:
    def __init__(self, learning_rate: float = 0.01, max_iter: int = 1000, l2_lambda: float = None):
        ...
        self.l2_lambda = l2_lambda if l2_lambda is not None else 0.0
        self.losses = list()
    ...
    def fit(self, X: np.ndarray, y: np.ndarray) -> list[float]:
        ...
        for _ in range(self.max_iter):
            ...
            dw = (-2 * X.T @ error / n_samples) + (2 * self.l2_lambda * self.weights if self.l2_lambda else 0)
...
def tune_hyperparams_linear_regression(X_train: np.ndarray,
                                       y_train: np.ndarray,
                                       X_val: np.ndarray,
                                       y_val: np.ndarray,
                                       ...
                                       max_iter: int = 1000,
                                       target_rmse: float = 71.0
                                       ) -> Tuple[LinearRegression, float, float]:
    """
    Tune learning rate and L2 regularization strength for LinearRegression to reach target RMSE on validation data.
    ...
    Args:
        ...
        l2_lambdas: List of L2 regularization values to try
        max_iter: Maximum iterations for training
        target_rmse: Desired RMSE threshold on validation set

    Returns:
        Tuple of (best model, best_learning_rate, best_l2_lambda)
    """
    best_model = None
    best_rmse = float('inf')
    best_lr = None
    ...
    for lr in learning_rates:
        for l2 in l2_lambdas:
            model = LinearRegression(learning_rate=lr, l2_lambda=l2, max_iter=max_iter)
            model.fit(X_train, y_train)

            y_val_pred = model.predict(X_val)
            rmse = model.metric(y_val, y_val_pred)

            print(f"LR = {lr:.4f}, L2 = {l2:.5f} -> RMSE = {rmse:.4f}")
            ...
            if rmse <= target_rmse:
                print(f"Achieved target RMSE ≤ {target_rmse}: {rmse:.4f}")
                return model, lr, l2  # Early stop if goal met

    print(f"\nBest RMSE = {best_rmse:.4f} with LR = {best_lr:.4f} and L2 = {best_l2:.5f}")
    return best_model, best_lr, best_l2
...
def main():
    ...
    # Linear Regression
    # ========================================= #
    linear_model = LinearRegression(learning_rate=0.01, max_iter=1000, l2_lambda=0.0001)
    #linear_model = LinearRegression(learning_rate = 0.09, max_iter = 5000)
    linear_model.fit(X_train_scaled, y_train)

    learning_rates = [0.005, 0.01, 0.05, 0.1]
    l2_lambdas = [0.0, 0.001, 0.01, 0.1]
    
    best_model, best_lr, best_l2 = tune_hyperparams_linear_regression(
    ...
    print(f" Best config: LR={best_lr}, L2={best_l2}")
